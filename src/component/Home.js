...
import { CX_SEARCH, KEY_API_SEARCH, URL_DB, setItemLocalStorage, toastInfor, toastSuccess, toastSuccessz, LINK_SEARCH } from './base/base';
  ...
  return (
    <div>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={e => props.handleStart(handleClose)}>Post Key</Button>
...
function Home() {
    ...
    const timerId = setInterval(() => {
      ...
      fetch(`${URL_DB}WebClone/rd/xml/a/get-bai-viet-all`)
        ...
        .then(rs => {
          if (rs.length !== 0) {
            rs.map(async item => {
              await fetch(`${URL_DB}WebClone/rd/xml/a/get-url-by-id/${item.id_url}`)
                .then(response => response.json())
                .then(rss => {
                  baiVietAll.push({
                    url: rss[0].url,
  ...
  const cutKeyAndUrl = (myString) => {
    ...
    return [
      myString.slice(0, index),
      myString.slice(index + 1).trim()
    ]
  }
    ...
    newKeysArr.map(async (item, index) => {
      ...
      await fetch(`${URL_DB}WebClone/rd/xml/a/save-url/${item.id}`, options)
        ...
        .then(async rs => {
          ...
          await fetch(`${URL_DB}WebClone/rd/xml/a/check-key/${item.id}`, options2)
            .then(response => response.json())
            .then(rs2 => {
  ...
  return (
    <>
      <Box sx={{ position: 'relative', width: '22%', margin: '100px 0 0 120px' }}>
        <label htmlFor="inputTag">
          <span className="btn-input"> Import</span>
          <input id="inputTag" type="file" onChange={e => fileHandler(e)} style={{ "display": 'none' }} />
        </label>
                  ...
                  .map((data, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left" onClick={() => handleDisplayUrl(data.id)}>
                          {data.ten}
                        </TableCell>
      ...
      <TableUrl keyArr={file.rows} displayUrl={displayUrl} setDisplayUrl={setDisplayUrl} />
